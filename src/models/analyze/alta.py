...
def fat_analyze(dados):
    ...
    if (demand_med["np"]>demand_contr['np']):
        demand_np = demand_med["np"]
    else:
        demand_np = demand_contr['np']
        
    if (demand_med["fp"]>demand_contr['fp']):
        demand_fp = demand_med["fp"]
    else:
        demand_fp = demand_contr['fp']  
        
    value_demand_np = demand_np * taf["demanda_np"]["c_impost"]
    value_demand_fp = demand_fp * taf["demanda_fp"]["c_impost"]
    value_demand_utrap_np = (
        demand_ultrap_np * taf["demanda_ultrapassada_np"]["c_impost"]
    )
    value_demand_utrap_fp = (
        demand_ultrap_fp * taf["demanda_ultrapassada_fp"]["c_impost"]
    )
    ...
    value_reat_exc = (reat_exc["np"] + reat_exc["fp"]) * taf["reativo_exc_np"][
        "c_impost"
    ]
    multas_nf = multas["nf"]
    ...
    total = (
        ...
        + value_demand_utrap_np
        + value_demand_utrap_fp
        + value_demand_reatv_np
        + value_demand_reatv_fp
        + value_TUSD_np
        + value_TUSD_fp
        + value_TE_np
        + value_TE_fp
        ...
        + publ_light
        + imp_som_dim
        - ajuste_desconto_demand_np
        - ajuste_desconto_demand_fp
        - ajuste_desconto_cons
        + deb_servicos
    )
...
def analyse_CT(input):
    ...
    if (
        data["multas_nf"] != 0
        or data["multas_cosip"] != 0
        or data["parc"] != 0
        or data["value_demand_utrap_np"] != 0
        or data["value_demand_utrap_fp"] != 0
        or data["value_demand_reatv_np"] != 0
        or data["value_demand_reatv_fp"] != 0
        or data["value_reat_exc"] > 0.05 * data["total_fat"]
    ):
        multa = True
    else:
        multa = False
...
def analyse_ML(input):
    ...
    if (
        (data["total"] != data["total_fat"]
        and data["total_fat"] > 1.05 * data["total"])
        or data["value_TE_fp"] != 0
        or data["value_TE_np"] != 0
        or data["desc"] == 0
    ):
        fat_error = True
    else:
